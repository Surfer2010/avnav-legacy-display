...
L.format=function(v,item){var n,dec=parseInt(item.decimals,10);if(isNaN(dec))dec=1;if(item.formatter==='text')return v===null||typeof v==='undefined'?'--':String(v);n=Number(v);if(v===null||v===''||isNaN(n))return dec?'--,-':'--';switch(item.formatter){case'course':n=Math.round(n)%360;if(n<0)n+=360;return('00'+n).slice(-3);case'directionRad':n=n*180/Math.PI;n=Math.round(n)%360;if(n<0)n+=360;return('00'+n).slice(-3);case'angleRad':n=n*180/Math.PI;while(n>180)n-=360;while(n<-180)n+=360;return(n<0?'-':'+')+('00'+Math.abs(Math.round(n))).slice(-3);case'speedMpsKn':n*=1.943844;break;case'depthAdaptive':dec=n<1?2:(n<10?1:0);break;case'kelvin':n-=273.15;break;case'pascalHpa':n/=100;break;case'percent':n*=100;break;case'latitude':return L.nautical(n,'N','S',2);case'longitude':return L.nautical(n,'E','W',3);}return n.toFixed(dec).replace('.',',');};
